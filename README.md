# FeiXiang's Research Library

个人医学图像研究知识库，聚焦**医学图像分割单源域泛化**（Single-Source Domain Generalization）。

网站：[https://kangkang03jug.github.io/feixiang-research-library/](https://kangkang03jug.github.io/feixiang-research-library/)

站点由 [Research Library Template](https://github.com/kangkang03jug/research-library-template) 独立创建。当前论文池包括单源域形状先验、因果启发增强、频谱正则，以及 SAM2 双编码器和极少标注泛化等方向。每篇论文提供中文 Quick Read、研究问题、方法、实验结果、局限和后续阅读建议，并保留原论文链接及页码定位。

## 站点栏目

- **Paper Pool**：搜索、主题/年份/相关性筛选和排序。
- **Daily Archive**：按日期查看推荐论文；研究画像中的默认更新计划为每天 08:00（Asia/Shanghai）。
- **Quick Read 与详细报告**：快速摘要和可追溯的结构化论文解读。
- **Deep Read、Favorites 与 My Notes**：本地数据模型支持阅读状态、收藏、个人标签和笔记。

研究方向、论文筛选偏好和时区配置位于 `config/research-profile.yaml`；论文记录位于 `data/papers/`，推荐历史位于 `data/daily/`，个人阅读状态位于 `data/user/`。网页默认静态只读；自动更新与在线写回可按下方模板说明另行配置。
